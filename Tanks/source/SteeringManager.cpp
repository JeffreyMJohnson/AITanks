...
vec SteeringManager::DoWander()
{
	...
	*/
	...
	/*get facing direction vector of player dirVector*/
	vec dirVector;
	if (mHost->GetVelocity() == vec(0, 0))
	{
		dirVector = glm::normalize(vec((rand() % (int)(mHost->GetMaxVelocity()) + 1, (rand() % (int)mHost->GetMaxVelocity()) + 1)));
	}
	else
	{
		dirVector = glm::normalize(mHost->GetVelocity());
	}
